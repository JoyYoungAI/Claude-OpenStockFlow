...
const learningTopics = [
  {
    ...
    sections: [
      ...
      { heading: "單據狀態流程", items: ["草稿（draft）：建單中，可修改，不影響庫存", "已提交（submitted）：等待審核，不影響庫存", "已核准（approved）：通過初審，尚待最終確認", "已退回（rejected）：審核未通過，可修改後重新提交", "已確認（confirmed）：庫存與財務正式生效", "已作廢（voided）：庫存回復，留有稽核紀錄"], type: "info" },
      { heading: "示範情境", body: "示範資料以「海風咖啡」為背景，涵蓋過去 40 天的完整進銷存週期，包含應付逾期、應收到期、借貨流程與低庫存警示。", type: "info" }
    ]
  },
  {
    id: "approval-flow", title: "審核流程示範", summary: "了解草稿→提交→核准→確認的完整四階段流程。",
    sections: [
      { heading: "為何需要審核流程？", body: "草稿與已提交的單據不計入庫存，可在確認前修正或取消，避免操作失誤造成帳務混亂。完整流程為：草稿 → 送審 → 核准 → 確認，缺少任一步驟都無法完成確認。", type: "info" },
  ...
  },
  {
    ...
    sections: [
      { heading: "主動借出（無原始銷售）", body: "使用調撥單從主倉移至「借出中」倉庫，並在備註記錄借貨對象與目的。", type: "info" },
      { heading: "銷售轉借貨（已有銷售單）", body: "確認後的銷售單，若客戶需轉為借貨，可對單身明細點擊「轉借貨」，系統會原子化地建立銷售退貨（回補庫存、核減應收）並自動調撥至借貨倉。", type: "info" },
