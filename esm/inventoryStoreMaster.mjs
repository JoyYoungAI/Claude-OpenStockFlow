import { nonNegativeNumber, normalizeText as normalizeTextFromUtils } from "./inventoryUtils.mjs";
import {
...
function createMasterModule(ctx) {
  ...
  // Store-master product normalization is intentionally local for CJS parity.
  ...
  // Store-master product copying is intentionally local for CJS parity.
  ...
  function listProducts(options) {
    ...
    const query = normalizeTextFromUtils(filter.query).toLowerCase();
    return getProducts()
...
const inventoryStoreMaster = { createMasterModule };

export { createMasterModule };
export default inventoryStoreMaster;
