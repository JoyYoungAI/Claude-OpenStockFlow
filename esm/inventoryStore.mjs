...
import {
  ...
  exportInventoryRows as reportExportInventoryRows,
  grossProfitRanking as reportGrossProfitRanking,
  inventoryReport as reportInventoryReport,
  productWarehouseSummary as reportProductWarehouseSummary,
  reportSummary as reportReportSummary,
  stockForProduct as reportStockForProduct,
  stockMovements as reportStockMovements,
...
function createInventoryStore(initialState) {
  ...
  function stockForProduct(productId, warehouseId) {
    return reportStockForProduct(reportState(), productId, warehouseId);
  }
  ...
  const financeModule = createFinanceModule(ctx);
  const masterCtx = Object.assign({}, ctx, {
    findProduct,
    setProductsCost
  });
  const masterModule = createMasterModule(masterCtx);
  ...
  function inventoryReport(options) {
    return reportInventoryReport(reportState(), options);
  }

  function dashboard() {
    return reportDashboard(reportState());
  }

  function grossProfitRanking(limit) {
    return reportGrossProfitRanking(reportState(), limit);
  }

  function warehouseStockSummary(options) {
    return reportWarehouseStockSummary(reportState(), options);
  }

  function productWarehouseSummary(options) {
    return reportProductWarehouseSummary(reportState(), options);
  }

  function warehouseTransferSummary(options) {
    return reportWarehouseTransferSummary(reportState(), options);
  }

  function reportSummary(options) {
    return reportReportSummary(reportState(), options);
  }

  function stockMovements(options) {
    return reportStockMovements(reportState(), options);
  }

  function exportInventoryRows(options) {
    return reportExportInventoryRows(reportState(), options);
  }
