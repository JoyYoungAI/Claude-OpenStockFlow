...
import inventoryModelsMaster, {
  ...
  sameSku,
  sameWarehouse
} from "./inventoryModelsMaster.mjs";
...
function loadPurchaseDocs(items) {
  ...
  if (Array.isArray(items[0] && items[0].lines)) { return items.map(copyPurchaseDoc); }
  if (items[0] && items[0].productId !== undefined) { return groupFlatPurchasesToDocs(items); }
  return items.map(copyPurchaseDoc);
}

function loadSaleDocs(items) {
  if (!Array.isArray(items) || !items.length) { return []; }
  if (Array.isArray(items[0] && items[0].lines)) { return items.map(copySaleDoc); }
  if (items[0] && items[0].productId !== undefined) { return groupFlatSalesToDocs(items); }
...
const inventoryModels = Object.assign(
  {},
